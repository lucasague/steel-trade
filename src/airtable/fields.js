export const FACTURA = {
  ID: "fldltLIbwNuH9RWo9",
  FECHA: "fldTOhx5UyOR4IGS6",
  VALOR_FACTURA_SIN_IVA: "fldpUg71bnByRXn4i",
  TIPO: "fldT47Q71LIWITRo7",
  DESCRIPCION: "fld7jfExgeRkJJwDw",
  VENTA: "fldB6zFs0qcnmA5gO",
  INCOTERM: "fldlOSfyhSnK7RE8o",
  PESO: "fld5HTNTpHqGXTgiT",
  TERMINOS_PAGO: "fld2RZDepeJvHVo4n",
  EXISTENCIAS: "fld76m6DpeR3OaulN",
  VALOR_VENTA: "fldjZ6fi8ZiCYep0n",
  PACKING_LIST: "fldyUtrc1lXmY5gAJ",
  CLIENTE: "fldHNvify7Ab945xT",
  CLIENTE_NOMBRE_FISCAL: "fldS9jkgx94V74CRT",
  CLIENTE_DOMICILIO: "fldNvQvMgVd4swnlZ",
  CLIENTE_CP: "fldtZUuulg5wCPtYu",
  CLIENTE_MUNICIPIO: "fldBR1V59yFMWf6GF",
  CLIENTE_PROVINCIA: "fldqeE34Dwg2UWc8c",
  CLIENTE_PAIS: "fldg47CYjDNI2t31T",
  CLIENTE_NIF: "fldOr31AI2jXkvCou",
  VENCIMIENTO_ESTIMADO: "fld47AWj5BCNOql1J"
};

export const EXISTENCIA = {
  ID_FABRICA: "fldCMmf7cTUzUmPAb",
  NETO: "fldvpgxVSxQWzOv6X",
  BRUTO: "fldD4itGy9e1qTIxT",
  PESO_FACTURA: "fldjnWnawgEqOIKgu",
  ESPESOR: "fldltcPdAI7UCKCIi",
  ANCHO: "fldGKYFeLwtZB9nz9",
  LARGO: "fld0DAEbd7fQabBCa",
  MEDIDA: "fldJpUP2JCAwmLIdo",
  MATERIAL: "fldk6qFHHgXn1gXk2",
  CALIDAD: "fldVtpMWSNzzXTkxs",
  DESCRIPCION: "fldx8rDlQDuRZsLY9",
  DESCRIPCION_DETALLE: "fldCzDd5UMrc0UZo5",
  PRECIO_EUR_MT: "fldxMzVEgXDZiTj2P",
  PACKING_LIST: "fldKrns7X3mkA8R9a",
  PACKING_LIST_LINK: "fld6tmYuaDmneCuKu",
  ITEM_VENTA: "fld6aouedO2bl0MuG",
  ITEM_COMPRA: "fld1LfDllQPTAX466",
  FACTURA: "fldsU4N8lvdySqsn0",
  TIPO_EXISTENCIA: "fld9jN4CH3KaXAyxc",
  NETO_BRUTO_CLIENTE: "fldOeiDrQFe5AO1Rf",
  MONEDA: "flddY6XIvf9FnmXH4",
  UDS: "fldnUz9FK7PynVPtC"
};

export const CONTRATO_VENTA = {
  CONTRATO: "fldBxZ0nEJAOnGiUj",
  NUMERO_PEDIDO: "fld70YdIwfeD4F1jD",
  FECHA: "fldmLylC7OzxfGfsL",
  CLIENTE: "fld2DzC3zydqWMAj7",
  INCOTERM: "fldUag142RE90yucx",
  ULTIMA_FECHA_EMBARQUE: "fldjNjBRwCpWSAONI",
  TOLERANCIA_MENOS: "fldIGNZFIIW0n5zt5",
  TOLERANCIA_MAS: "fldyvgrgAA0HYgDSn",
  MONEDA: "fldfAYJqypZS9nIlw",
  PAGO: "fldX3lewbUGpXejsL",
  ITEMS: "fldSSpBbTESUG8Z2T",
  DOCUMENTOS_CONFIRMACION: "fld1bqIxDKpu8mEmM",
  CLIENTE_NOMBRE_FISCAL: "fldHRTcDSJ49n1sey",
  CLIENTE_DOMICILIO: "fldjtk6h06q9lHNXk",
  CLIENTE_CP: "fld3JZYE65iAvoWPS",
  CLIENTE_MUNICIPIO: "fld8IcffY5cAeDH1g",
  CLIENTE_PROVINCIA: "fld2sDQEGk7t1ypIv",
  CLIENTE_PAIS: "fldykRwjTn26WqRuu",
  CLIENTE_NIF: "fldaxgBqeiNKk2K70",
  PESO_POR_CONTRATO: "fldYBGnNy4Nt6U6An",
  TERMINOS_PAGO: "fldvdwoYOJVEHjRrx"
};

export const ITEM_VENTA = {
  ITEM: "fldVvxDlxNOXZ7Q0J",
  NUMERO_ITEM: "flddt8lnZh8hMVLKj",
  CONTRATO: "fldvHy8aYYfGDJNph",
  ITEM_COMPRA: "fldqKI4x1KDXQE6fr",
  PESO_CONTRATO_BOBINAS: "fldVv2L7Ns2moxvmM",
  NETO_CARGADO: "fldHi7YZVVajiFLDW",
  BRUTO_CARGADO: "fld6rigMd5I56t6Nv",
  PESO_CARGADO: "fldx2X5SpVrUbcA4Q",
  EXISTENCIAS: "fldQ4R2fkBDAX5TIj",
  PRECIO: "fldIuPEBk922fo5aK",
  MONEDA: "fldNkIDFk5kyXTUFK",
  PRECIO_EUR: "fldcg1jTAyKcxeKGC",
  VALOR_CONTRATO_EUR: "flduhhirgd8KXz8dA",
  VALOR_EXISTENCIAS_EUR: "flda7edE1LIC3gKbI",
  NUMERO_CHAPAS: "fldFDu8QuVIKZIrMz"
};

export const ITEM_COMPRA = {
  ITEM: "fld0CH2JzvCx1Ly8x",
  MATERIAL: "fldnrspm1JWHaxr3r",
  TIPO_MATERIAL: "fld4lkih5T3Omocet",
  CALIDAD: "fldAqQp8l2ik11eMW",
  RECUBRIMIENTO: "fld9JE3gBDZygDARd",
  ACABADO: "fldUSfTUqoqVJqytG",
  ESPESOR: "fldt6DaiZr6TqbCe3",
  ANCHO: "fldIqWvVCqo1mW8sR",
  LARGO: "fldiW21k2rkbPGV37",
  NETO_MINIMO: "fldzGRqBgc129dCHy",
  NETO_MAXIMO: "fldiKX4sb52oiN4or",
  NETO_CONTRATO: "fld1ruzJNlsYCl951",
  PRECIO: "fldIHNan1nQF63kvI",
  PRECIO_EUR: "fld9kv9blMfigCcI3",
  EXISTENCIAS: "fldWAIzVwE8nqvdHL"
};

export const FACTURA_FIELD_IDS = Object.values(FACTURA);
export const EXISTENCIA_FIELD_IDS = Object.values(EXISTENCIA);
export const CONTRATO_VENTA_FIELD_IDS = Object.values(CONTRATO_VENTA);
export const ITEM_VENTA_FIELD_IDS = Object.values(ITEM_VENTA);
export const ITEM_COMPRA_FIELD_IDS = Object.values(ITEM_COMPRA);
